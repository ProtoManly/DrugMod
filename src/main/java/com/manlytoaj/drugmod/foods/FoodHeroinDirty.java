...
public class FoodHeroinDirty extends ItemFood implements IHasModel {
	public FoodHeroinDirty(String name, int amount, float saturation, boolean isWolfFood) {
	...
	public ItemStack onItemUseFinish(ItemStack stack, World worldIn, EntityLivingBase player) {
		if(!worldIn.isRemote) {
			Heroin.addEffect(player);
			DirtyNeedle.addEffect(player);
		}
		EntityPlayer plr = (EntityPlayer)player;
		if(!plr.isCreative()){
			plr.addItemStackToInventory(new ItemStack(ModItems.USED_NEEDLE, 1));
			stack.shrink(1);
		}
		return(stack);
	}
